    ...
    function deliver_Svc_WriteRes(msg : Write_Res): bool
        ...
        */
        ...
        if (msg->result_size() > 0) {
            ...
            write_response->Assign(WRITE_RES_STATUS_CODE_LINK_ID_SRC_IDX, zeek::make_intrusive<zeek::StringVal>(result_idx));
            for (int i = 0; i < msg->result_size(); i++) {
                generateStatusCodeEvent(connection(), write_response->GetField(WRITE_RES_STATUS_CODE_LINK_ID_SRC_IDX), StatusCode_WriteResponse_Key, msg->results()->at(i), status_code_level, msg_header->is_orig());
            }
        }
        ...
        if (msg->diagnostic_info_size() > 0) {
            ...
            for (int i = 0; i < msg->diagnostic_info_size(); i++) {
                ...
                generateDiagInfoEvent(connection(), write_res->GetField(WRITE_RES_DIAG_INFO_LINK_ID_SRC_IDX), msg->diagnostic_info()->at(i), stringTable, innerDiagLevel, StatusCode_WriteResponse_DiagInfo_Key, msg_header->is_orig(), DiagInfo_WriteResponse_Key);
            }
        }
